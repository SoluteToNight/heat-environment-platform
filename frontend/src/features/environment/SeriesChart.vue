...
<template>
  ...
  <div v-if="!table" class="relative mt-2">
    ...
    <label class="mt-3 flex items-center justify-between text-xs text-muted">查看时刻<select class="max-w-36 rounded border border-line bg-white px-2 py-2 text-ink" :value="current || ''" @change="emit('choose', ($event.target as HTMLSelectElement).value)"><option value="" disabled>选择时间</option><option v-for="point in series.points" :key="point.time" :value="point.time" :disabled="point.value === null || !times.includes(point.time)">{{ localTime(point.time, true) }}</option></select></label>
  </div>
